...
file_path = "test-files\DJ Trapezoid - Pumpkin Head.aseprite"

PRINT "=== CEL LAYER INDEX VERIFICATION (CORRECT FILE) ==="
...
' Check frame 0 for pumpkin head file
frame_num = 0
...
FOR chunk_num = 1 TO num_chunks
    ...
    ELSE
        ' Skip non-CEL chunks
        PRINT "Non-CEL chunk type: 0x"; HEX$(chunk_header.chunk_type)
        next_chunk_pos = chunk_start_pos + chunk_header.chunk_size
        SEEK #file_handle, next_chunk_pos
    END IF
NEXT chunk_num
