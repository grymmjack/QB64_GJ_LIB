''
' test_layer_extraction.bas - Simple test for ASEPRITE layer extraction
'
...
' Test 1: Extract layer 0 (BG) - has actual pixel data
_ECHO "Test 1: Extracting layer 0 (BG) - has actual pixel data"
DIM bg_img AS LONG
bg_img = create_aseprite_image_from_layer(TEST_FILE$, "BG", 0, 0)
IF bg_img <> 0 THEN
    _ECHO "SUCCESS: Layer 'BG' extracted (Handle: " + STR$(bg_img) + ")"
...
' Test 2: Extract layer 7 (Pumpkin Head) - empty layer
_ECHO "Test 2: Extracting layer 7 (Pumpkin Head) - empty layer"
DIM pumpkin_img AS LONG
pumpkin_img = create_aseprite_image_from_layer(TEST_FILE$, "Pumpkin Head", 7, 0)
IF pumpkin_img <> 0 THEN
    _ECHO "SUCCESS: Layer 'Pumpkin Head' extracted (Handle: " + STR$(pumpkin_img) + ")"
    _ECHO "  Dimensions: " + STR$(_WIDTH(pumpkin_img)) + "x" + STR$(_HEIGHT(pumpkin_img))
    _ECHO "  Note: This layer was empty, so a transparent image was created"
    _ECHO "  (You would see nothing when displaying this image)"
    _FREEIMAGE pumpkin_img
ELSE
    _ECHO "FAILED: Could not extract layer 'Pumpkin Head'"
END IF
_ECHO ""

' Test 3: Extract non-existent layer
_ECHO "Test 3: Extracting non-existent layer"
DIM nonexist_img AS LONG
nonexist_img = create_aseprite_image_from_layer(TEST_FILE$, "NonExistent", -1, 0)
IF nonexist_img <> 0 THEN
    _ECHO "UNEXPECTED: Non-existent layer extracted (Handle: " + STR$(nonexist_img) + ")"
    _FREEIMAGE nonexist_img
ELSE
    _ECHO "EXPECTED: Non-existent layer correctly returned 0 (failure)"
END IF
_ECHO ""

' Test 4: Test merged layers function
_ECHO "Test 4: Creating merged image from all layers"
DIM merged_img AS LONG
merged_img = create_aseprite_image_from_merged_layers(TEST_FILE$, 0)
IF merged_img <> 0 THEN
    _ECHO "SUCCESS: Merged image created (Handle: " + STR$(merged_img) + ")"
    _ECHO "  Dimensions: " + STR$(_WIDTH(merged_img)) + "x" + STR$(_HEIGHT(merged_img))
    _FREEIMAGE merged_img
ELSE
    _ECHO "FAILED: Could not create merged image"
END IF

_ECHO ""
_ECHO "Layer extraction tests completed!"
SYSTEM
...
''
' Create an ASEPRITE image from merged layers
'
